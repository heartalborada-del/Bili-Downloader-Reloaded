...
internal class TerminalProcessProgress(
    ...
    fun updateTxt(terminalWidth: Int) {
...
private val terminalDisplay: Display by object : kotlin.properties.ReadOnlyProperty<Any?, Display> {
    val delegate: () -> Display by lazy {
        val lr = CliMain.getLineReader()
...
private val lineReaderMaskingCallback: Field by lazy {
...
private val lineReaderReadingField: Field by lazy {
    val field = LineReaderImpl::class.java.getDeclaredField("reading")
    field.isAccessible = true
    field
}

private val terminalExecuteLock: java.util.concurrent.locks.Lock by lazy {
    val lr = CliMain.getLineReader()
    val field = LineReaderImpl::class.java.declaredFields.first {
...
private val terminalDownloadingProgresses = ConcurrentLinkedQueue<TerminalProcessProgress>()
...
fun updateTerminalDownloadingProgresses() {
    ...
    terminalExecuteLock.withLock {
        CliMain.getTerminal().pause(false)

        if (terminalDownloadingProgresses.isNotEmpty()) {
            val wid = CliMain.getTerminal().width
            ...
            (CliMain.getLineReader() as LineReaderImpl).let { lr ->
                if (lr.isReading) {
                    lr.redisplay()
                }
            }
            noticeDownloadingProgressEmpty()
            CliMain.getTerminal().writer().print("\u001B[?25h") // show cursor
        }
    }
}

private fun printToScreen(msg: String) {
    if (!containDownloadingProgress) {
        if (msg.endsWith(ANSI_RESET)) {
            CliMain.getLineReader().printAbove(msg)
        } else {
            CliMain.getLineReader().printAbove(msg + ANSI_RESET)
        }
        return
    }
    terminalExecuteLock.withLock {
...
private fun printToScreen(msg: AttributedString) {
    if (!containDownloadingProgress) {
        return CliMain.getLineReader().printAbove(msg)
    }
    terminalExecuteLock.withLock {
        terminalDisplay.update(emptyList(), 0)
...
private fun waitDownloadingProgressEmpty() {
    synchronized(terminalDownloadingProgressesNoticer) {
        if (containDownloadingProgress) {
